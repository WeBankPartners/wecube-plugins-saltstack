...
func StartCleanInterval() {
	...
	intervalSecond := 3600
	timeStartValue, _ := time.Parse("2006-01-02 15:04:05 MST", fmt.Sprintf("%s:00:00 "+localTimeZone, time.Now().Format("2006-01-02 15")))
	time.Sleep(time.Duration(timeStartValue.Unix()+3600-time.Now().Unix()) * time.Second)
...
func cleanLocalPackage(dirPath string) {
	...
	minUnixTime := time.Now().Unix() - 10800
	for _, f := range files {
