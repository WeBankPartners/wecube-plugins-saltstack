...
push: image
	...
	docker tag $(project_name):$(version) $(dockerhub_server)/$(dockerhub_path)/wecube-statstack:$(version)
	docker push $(dockerhub_server)/$(dockerhub_path)/wecube-statstack:$(version)

run_container: push
	docker -H $(server_addr) login -u $(dockerhub_user) -p $(dockerhub_pass) $(dockerhub_server)
	docker -H $(server_addr) rm -f $(docker ps -a | grep "wecube-statstack-smoke" | awk '{print $1}') | true
